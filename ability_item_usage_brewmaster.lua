...
Consider[1] = function()
	...
	--------------------------------------
	--protect myself
	local blademailEnemies = AbilityExtensions:Count(enemys,
		function(t) return t:HasModifier("modifier_item_blade_mail") end)
...
local function EnemyCloseEnoughToUsePrimalSplit(t)
	return GetUnitToUnitDistance(npcBot, t) <= 600 or
...
Consider[5] = function()
	...
	-- If we're in a teamfight, use it on the scariest enemy
	local tableNearbyAttackingAlliedHeroes = AbilityExtensions:GetPureHeroes(npcBot, 1000):Filter(function(t) AbilityExtensions
			:IsAttackingEnemies(t)
	end)
