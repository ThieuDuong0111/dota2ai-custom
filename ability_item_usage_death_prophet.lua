...
Consider[2] = function()
	...
	enemys = AbilityExtensions:Filter(enemys,
		function(t) return not t:IsStunned() and not t:IsSilenced() and not t:IsInvulnerable() and not t:IsMagicImmune() end)
	local WeakestEnemy, HeroHealth = utility.GetWeakestUnit(enemys)
		...
		then
			...
			then
				return BOT_ACTION_DESIRE_MODERATE,
					utility.GetUnitsTowardsLocation(npcBot, npcEnemy, GetUnitToUnitDistance(npcBot, npcEnemy) + 100)
			end
		end
	end
