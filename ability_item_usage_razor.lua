...
Consider[1] = function()
	...
	then
		...
		then
			if (
				(ManaPercentage > 0.75 or npcBot:GetMana() > ComboMana) and
	...
	then
		...
		then
			if (
				CanCast[abilityNumber](npcEnemy) and GetUnitToUnitDistance(npcBot, npcEnemy) < Radius and
					GetUnitToUnitDistance(npcBot, npcEnemy) > 0.5 * Radius)
			then
