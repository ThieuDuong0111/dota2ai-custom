...
Consider[2] = function()
	...
	then
		...
		if (
			npcTarget ~= nil and t ~= nil and npcBot:GetMana() >= AbilitiesReal[2]:GetManaCost() + AbilitiesReal[1]:GetManaCost()
			)
		then
...
Consider[5] = function()
	...
	if AbilityExtensions:HasScepter(npcBot) then
		if not npcBot:IsHexed() and AbilityExtensions:IsSeverelyDisabled(npcBot) and npcBot:GetMana() >= ability:GetManaCost()
			and #AbilityExtensions:GetNearbyNonIllusionHeroes(1599) > 0 then
			return BOT_ACTION_DESIRE_HIGH
		end
	end
