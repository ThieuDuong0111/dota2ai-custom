...
Consider[1] = function() --Location AOE Example
	...
	then
		...
		then
			local locationAoE = npcBot:FindAoELocation(true, true, npcBot:GetLocation(), CastRange, Radius, 0, Damage);
...
Consider[2] = function() --Location AOE Example
...
local function FindNearbyTeleportTarget(target)
	...
	local creeps = AbilityExtensions:GetNearbyCreeps(target, 600, isEnemy)
		:Filter(function(t) return not t:WasRecentlyDamagedByAnyHero(3) and
				#AbilityExtensions:GetNearbyHeroes(t, 500, true, BOT_MODE_NONE) == 0
		end)
	...
	buildings = AbilityExtensions:Filter(buildings,
		function(t) return t:HasModifier("modifier_backdoor_protection_active") and t:GetHealth() >= 100 or
				t:GetHealth() >= 300 and not t:WasRecentlyDamagedByAnyHero(castDelay - 2) or t:GetHealth() >= 800 or
	...
	if AbilityExtensions:HasScepter(npcBot) then
		...
		friends = AbilityExtensions:Filter(friends,
			function(t) return t:NotRetreating(t) and AbilityExtensions:AllyCanCast(t) and
					AbilityExtensions:GetHealthPercent(t) >= 0.5 and not t:IsSeverelyDisabled(t)
			end)
		friends = AbilityExtensions:FilterNot(friends,
			function(t) return t:IsIllusion() and
					AbilityExtensions:GetModifierRemainingDuration(t, "modifier_illusion") <= castDelay
			end)
...
Consider[5] = function()
	...
	if darkRiftChosenTarget then
		if darkRiftOriginalTarget:IsAlive() then
			...
			if GetUnitToLocationDistanceSqr(darkRiftChosenTarget, trackOriginalTargetPosition) >= darkRiftCancelRange and
				(
				target == nil or
					target:CanBeSeen() and GetUnitToLocationDistanceSqr(darkRiftChosenTarget, target) >= darkRiftCancelRange) then
