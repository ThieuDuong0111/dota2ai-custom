...
local CanCast = { AbilityExtensions.PhysicalCanCastFunction, function(t)
	if npcBot:GetTeam() == t:GetTeam() then
		return AbilityExtensions:SpellCanCast(t, true, true, true) and not AbilityExtensions:DontInterruptAlly(t) and
			not t:IsMagicImmune()
	else
		return AbilityExtensions:NormalCanCast(t, false, DAMAGE_TYPE_MAGICAL) and
			not t:HasModifier("modifier_antimage_counterspell") and not AbilityExtensions:IsOrGoingToBeSeverelyDisabled(t)
	end
end, nil,
	function(t)
...
local dazzle_shadow_grave = function()
	...
	for _, npcTarget in pairs(allys) do
		if (npcTarget:GetHealth() / npcTarget:GetMaxHealth() <= 0.2 + 0.05 * enemyCount) and
			AbilityExtensions:IsOrGoingToBeSeverelyDisabled(npcTarget) and CanCast[2](npcTarget)
		then
		...
		then
			if (
				CanCast[abilityNumber](npcTarget) and not AbilityExtensions:IsOrGoingToBeSeverelyDisabled(npcTarget) and
					GetUnitToUnitDistance(npcBot, npcTarget) > 75 * #allys
...
Consider[5] = function()
	...
	then
		...
		if (#enemys >= 2)
		then
			if npcBot:GetModifierStackCount(npcBot:GetModifierByName("modifier_obsidian_destroyer_astral_imprisonment_charge_counter"))
				> 30
				or
				(
				#enemys >= 3 and
					npcBot:GetModifierStackCount(npcBot:GetModifierByName("modifier_obsidian_destroyer_astral_imprisonment_charge_counter"))
					> 12)
			then
