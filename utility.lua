	...
	for _, enID in pairs(GetTeamPlayers(GetTeam())) do
		...
		if enemyInfo ~= nil and enemyInfo['location'] ~= nil then
			if IsHeroAlive(enID) and utilityModule.GetDistance(enemyInfo['location'], loc) <= dist and
				(utilityModule.GetDistance(enemyInfo['location'], Vector(0, 0)) > 10) and enemyInfo['time_since_seen'] < 10 then
...
utilityModule.FillInAbilities = function(npcBot, abilityTable)
	...
	for i = 1, 30 do
		...
		elseif not AbilityExtensions:Contains(abilities, abilityTable[i]) then
			print("Bot script " .. npcBot:GetUnitName() .. " contains incorrect ability level " .. i .. " name: " ..
				abilityTable[i])
			abilityTable[i] = AbilityExtensions.IncorrectAbilityName
		end
	end
	...
	abilityTable.talentTreeIndex = AbilityExtensions:Count(abilityTable, function(abilityName, index)
		return index < npcBot:GetLevel() - npcBot:GetAbilityPoints() + 1 - abilityTable.incorrectAbilityLevelUpNumber and
			abilityName == "talent"
	end)
end
...
function utilityModule.IsStuck(npcBot)
	if npcBot.stuckLoc ~= nil and npcBot.stuckTime ~= nil then
		...
		if npcBot:GetCurrentActionType() == BOT_ACTION_TYPE_MOVE_TO and attackTarget == nil and EAd > 2200 and TAd > 2200 and
			#Et == 0 and #At == 0
