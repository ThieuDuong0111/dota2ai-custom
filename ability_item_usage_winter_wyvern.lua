...
local CanCast = { utility.NCanCast, utility.NCanCast, function(t)
	if not AbilityExtensions:AllyCanCast(t) or
		AbilityExtensions:MustBeIllusion(npcBot, t) and not AbilityExtensions:IsTempestDouble(t) or
		not AbilityExtensions:IsHero(t) and not AbilityExtensions:IsLoneDruidBear(t) then
		return false
	end
	...
	return AbilityExtensions:AllyCanCast(t) and
		(
		not AbilityExtensions:DontInterruptAlly(t) or
			t:HasModifier("modifier_medusa_stone_gaze") and t:GetActiveMode() == BOT_MODE_RETREAT) and not t:IsChanneling()
end, function(t)
...
Consider[1] = function()
	...
	then
		for i, npcEnemy in pairs(enemys) do
			if (
				npcBot:HasModifier("modifier_winter_wyvern_arctic_burn_flight") and
					npcEnemy:HasModifier("modifier_winter_wyvern_arctic_burn_slow"))
			then
...
Consider[3] = function()
	...
	allys = AbilityExtensions:Filter(allys, function(t)
		return AbilityExtensions:MayNotBeIllusion(npcBot, t) and not AbilityExtensions:CannotBeTargetted(t) and
			not AbilityExtensions:IsInvulnerable(t) and not AbilityExtensions:CannotBeKilledNormally(t)
	end)
	...
	then
		...
		for _, npcTarget in pairs(allys) do
			...
			if (
				npcTarget:GetHealth() / npcTarget:GetMaxHealth() < healingFactor and npcTarget:WasRecentlyDamagedByAnyHero(2.0) and
					npcTarget:GetActiveMode() ~= BOT_MODE_ATTACK)
			then
	...
	then
		for _, npcTarget in pairs(allys) do
			if npcTarget:GetHealth() / npcTarget:GetMaxHealth() < (0.35 + 0.4 * ManaPercentage) and
	...
	then
		...
		for i, npcEnemy in pairs(enemys) do
			...
			then
				if (enemys2 ~= nil)
				then
					for j, npcEnemyAttacker in pairs(enemys2) do
						if (npcEnemy ~= npcEnemyAttacker)
						then
							sumdamage = sumdamage +
								npcEnemyAttacker:GetEstimatedDamageToTarget(true, npcEnemy, Duration, DAMAGE_TYPE_PHYSICAL)
						end
					end
				end
				for j, npcEnemyAttacker in pairs(creeps2) do
