...
local CanCast = { utility.NCanCast, function(t)
	...
	return AbilityExtensions:NormalCanCast(t, true,
		AbilityExtensions:HasScepter(npcBot) and DAMAGE_TYPE_PURE or DAMAGE_TYPE_MAGICAL, false, false)
end }
				...
				then
					...
					then
						...
						return BOT_ACTION_DESIRE_HIGH,
							utility.GetUnitsTowardsLocation(npcBot, WeakestEnemy, GetUnitToUnitDistance(npcBot, WeakestEnemy) + 200);
					end
				end
			end
		end
...
Consider[3] = function()
	...
	if npcBot:GetActiveMode() == BOT_MODE_PUSH_TOWER_TOP or
		...
		if tower then
			if A.Building.CanBeAttacked(tower) and tower:GetAttackTarget() and tower:GetAttackTarget() ~= npcBot or
				GetUnitToUnitDistanceSqr(npcBot, tower) >= 250000 then
...
Consider[5] = function()
	...
	local function GetDamage(target)
		local damageAtLeast = damagePerSecond *
			(lingerTime + AbilityExtensions:GetStunRemainingDuration(target) + pathRadius / target:GetCurrentMovementSpeed())
			...
			then
				...
				then
					...
					then
						...
						return BOT_ACTION_DESIRE_HIGH,
							utility.GetUnitsTowardsLocation(npcBot, WeakestEnemy, GetUnitToUnitDistance(npcBot, WeakestEnemy) + 100);
					end
				end
			end
		end
