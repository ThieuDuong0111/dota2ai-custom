...
local function ShouldSurge(t)
	return (
		AbilityExtensions:IsRetreating(t) or AbilityExtensions:GetHealthPercent(t) <= 0.4 or t:IsHexed() or
			t:GetCurrentMovementSpeed() <= 150) and AbilityExtensions:CanMove(t) and t:WasRecentlyDamagedByAnyHero(3)
end
...
Consider[3] = function()
	...
	if (locationAoE.count >= 2 and AOESurge:IsTrained() == true) and #enemys ~= 0 then
		return RemapValClamped(locationAoE.count, 2, 4, BOT_ACTION_DESIRE_LOW, BOT_ACTION_DESIRE_HIGH), locationAoE.targetloc,
			"Location"
	end
	--------------------------------------
	-- Mode based usage
