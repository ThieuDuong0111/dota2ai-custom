...
Consider[2] = function()
	...
	local imprisonedEnemy = AbilityExtensions:Map(npcBot:GetNearbyHeroes(CastRange + 100, true, BOT_MODE_NONE),
...
local ShardLagunaBlade = function()
	...
	if oDesire > 0 then
		...
		local guess = AbilityExtensions:Map(degree,
			function(t) return { t, myLocation + Vector(distance * math.cos(t), distance * math.sin(t)) } end)
		...
		if count[3] > 1 or AbilityExtensions:HasAbilityRetargetModifier(oTarget) or
			AbilityExtensions:CannotBeTargetted(oTarget) then
