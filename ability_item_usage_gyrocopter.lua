...
Consider[3] = function()
	...
	local CastRange = npcBot:GetAttackRange();
	local Damage = ability:GetAbilityDamage();
	local Radius = ability:GetAOERadius()
	local CastPoint = ability:GetCastPoint();
...
Consider[5] = function()
	...
	local Damage = ability:GetAbilityDamage();
	local Radius = ability:GetAOERadius()
	local CastPoint = ability:GetCastPoint() + 1.5;
	...
	then
		local locationAoE = npcBot:FindAoELocation(true, false, npcBot:GetLocation(), CastRange, Radius, 0, 0);
		local locationAoE2 = npcBot:FindAoELocation(true, true, npcBot:GetLocation(), CastRange, Radius, 0, 0);

		if (locationAoE.count >= 4 and locationAoE2.count >= 2)
		then
			return BOT_ACTION_DESIRE_LOW, locationAoE.targetloc;
		end
	end
