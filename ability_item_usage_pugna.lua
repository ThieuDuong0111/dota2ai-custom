...
local function CanCast2(t)
	if AbilityExtensions:IsOnSameTeam(npcBot, t) then
		...
		return AbilityExtensions:NormalCanCast(t) and
			not AbilityExtensions:GetNearbyNonIllusionHeroes(npcBot, 600, false):Any(function(t)
...
Consider[3] = function()
	...
	if (npcBot:GetActiveMode() == BOT_MODE_PUSH_TOWER_TOP or
		...
		npcBot:GetActiveMode() == BOT_MODE_DEFEND_TOWER_BOT)
	then
		if (tower ~= nil and GetUnitToUnitDistance(npcBot, tower[1]) >= 800) or
			(tower2 ~= nil and GetUnitToUnitDistance(npcBot, tower2[1]) >= 400 and #enemys >= 1)
		then
...
Consider[5] = function()
	...
	then
		...
		then
			...
			then
				if (
					WeakestEnemy:GetHealth() / WeakestEnemy:GetMaxHealth() < 0.8 or WeakestEnemy:HasModifier("modifier_pugna_decrepify")
					)
