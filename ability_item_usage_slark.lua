...
local CanCast = { utility.NCanCast, function(t)
	return AbilityExtensions:StunCanCast(t, AbilitiesReal[2], false, false, true, false) and A.Unit.IsNotCreepHero(t)
		...
		then
			if (
				HeroHealth <= WeakestEnemy:GetActualIncomingDamage(Damage, DAMAGE_TYPE_MAGICAL) and
					HealthPercentage <= 0.40 + 0.05 * #enemys and GetUnitToUnitDistance(npcBot, WeakestEnemy) < CastRange)
			then
				return BOT_ACTION_DESIRE_HIGH
			end
		end
