...
Consider[1] = function()
    ...
    if fun1:HasScepter(npcBot) then
        ...
        if (fun1:IsAttackingEnemies(npcBot) or fun1:IsRetreating(npcBot)) and npcBot:GetMana() >= 120 then
            ...
            return #enemies >= 2 and fun1:GetManaPercent(npcBot) >= 0.3 or
                #enemies == 1 and fun1:GetManaPercent(npcBot) >= 0.6 or npcBot:WasRecentlyDamagedByAnyHero(1.5)
        end
        return 0
    end
    ...
    if npcBot:GetActiveMode() ~= BOT_MODE_RETREAT then
        if WeakestEnemy ~= nil then
            if CanCast[abilityNumber](WeakestEnemy) then
                if HeroHealth <= WeakestEnemy:GetActualIncomingDamage(Damage, DAMAGE_TYPE_PHYSICAL) or
    ...
    if npcBot:GetActiveMode() == BOT_MODE_FARM or npcBot:GetActiveMode() == BOT_MODE_LANING then
        if #creeps >= 2 then
            if WeakestCreep ~= nil then
                if CreepHealth <= WeakestCreep:GetActualIncomingDamage(Damage, DAMAGE_TYPE_PHYSICAL) and
...
Consider[4] = function()
    ...
    if enemyCount == 1 then
        return BOT_ACTION_DESIRE_MODERATE,
            fun1:FindAOELocationAtSingleTarget(npcBot, enemeis[1], castRange, radius, castPoint)
    end
