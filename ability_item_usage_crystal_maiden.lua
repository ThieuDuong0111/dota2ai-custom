...
local CanCast = { utility.NCanCast, function(t)
	return AbilityExtensions:StunCanCast(t, AbilitiesReal[2], false, true)
...
Consider[2] = function()
	...
	then
		if (
			CreepHealth2 >= 600 and not StrongestCreep:GetUnitName() == "npc_dota_neutral_mud_golem" and
				not StrongestCreep:IsAncientCreep() and npcBot:GetMana() > 240) and
			not (StrongestCreep:WasRecentlyDamagedByAnyHero(3) and not StrongestCreep:WasRecentlyDamagedByHero(npcBot, 3))
		then
...
Consider[6] = function()
	...
	then
		...
		then
			if (
				CanCast[abilityNumber](npcEnemy) and enemyDisabled(npcEnemy) and
					npcEnemy:GetHealth() <= npcEnemy:GetActualIncomingDamage(
