...
local CanCast = { function(t)
	return AbilityExtensions:NormalCanCast(t, false, DAMAGE_TYPE_MAGICAL, true) and A.Unit.IsNotCreepHero(t)
end, utility.UCanCast, utility.UCanCast, utility.UCanCast, utility.UCanCast, utility.UCanCast, }
...
Consider[1] = function()
	...
	then
		...
		then
			if (
				(ManaPercentage > 0.6 or npcBot:GetMana() > ComboMana) and
...
Consider[6] = function()
	...
	if (npcBot:GetActiveMode() == BOT_MODE_RETREAT and npcBot:GetActiveModeDesire() >= BOT_MODE_DESIRE_HIGH) and
		AbilityExtensions:GetHealthPercent(npcBot) >= 0.4
	then
		for _, npcEnemy in pairs(enemys) do
			if (npcBot:WasRecentlyDamagedByHero(npcEnemy, 2.0))
			then
	...
	if (#tableNearbyAttackingAlliedHeroes + #enemys >= 5 and #enemys >= 1)
	then
		return BOT_ACTION_DESIRE_MODERATE, enemys[1]:GetLocation()
	end
...
Consider[4] = function()
	...
	if AbilitiesReal[4]:GetCooldown() - AbilitiesReal[4]:GetCooldownTimeRemaining() > 5 or
		AbilitiesReal[4]:GetCooldown() - AbilitiesReal[4]:GetCooldownTimeRemaining() == 0
	then
		return BOT_ACTION_DESIRE_NONE, 0;
	end
