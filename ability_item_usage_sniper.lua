...
Consider[1] = function()
	...
	if (desire ~= 0)
	then
		for _, i in pairs(npcBot.Sniper1) do
...
Consider[6] = function()
	...
	local enemys = {}
	for i, enemy in pairs(enemys3) do
	...
	then
		...
		then
			...
			then
				if (
					HeroHealth + WeakestEnemy:GetHealthRegen() * CastPoint <=
						WeakestEnemy:GetActualIncomingDamage(Damage, DAMAGE_TYPE_MAGICAL)
						and GetUnitToUnitDistance(WeakestEnemy, npcBot) > AttackRange and
	...
	then
		...
		then
			if (
				CanCast[abilityNumber](npcEnemy) and GetUnitToUnitDistance(npcBot, npcEnemy) > AttackRange and
